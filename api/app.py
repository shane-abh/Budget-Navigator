...
from fastapi import FastAPI, HTTPException, UploadFile, File, Depends, Response, Cookie
...
# Must specify explicit origins for cookie-based authentication
app.add_middleware(
    CORSMiddleware,
    allow_origins=[
        "http://localhost:8000",
...
# Once a user reaches this limit, they must start a new session to continue
MAX_QUESTIONS_PER_SESSION = 10

# ==================== Authentication Configuration ====================
# JWT secret key - IMPORTANT: Set this in your .env file for production
...
def create_token(user_name: str, session_id: str) -> str:
    """
    Create a JWT token for the authenticated user.
    
    Args:
        user_name: The user's display name
        session_id: The session ID associated with this user
...
def verify_token(token: str) -> dict:
    ...
    try:
        ...
        raise HTTPException(status_code=401, detail="Token expired. Please register again.")
    except jwt.InvalidTokenError:
        raise HTTPException(status_code=401, detail="Invalid token. Please register again.")

async def get_current_user(
    auth_token: str = Cookie(None),
    token: str = None  # Fallback: accept token as query parameter for EventSource
) -> dict:
    """
    ...
    2. Query parameter 'token' (fallback for EventSource which has cookie issues)
    
    This function is used as a FastAPI dependency to protect endpoints.
    
    Args:
        auth_token: JWT token from the HttpOnly cookie (automatically extracted)
        token: JWT token from query parameter (fallback for EventSource)
    
    Returns:
        Decoded user information including name and session_id
    
    Raises:
        HTTPException: If not authenticated or token is invalid
    """
    # Try cookie first, then fall back to query parameter
    effective_token = auth_token or token
    
    if not effective_token:
        raise HTTPException(
            status_code=401, 
            detail="Not authenticated. Please register first."
        )
    return verify_token(effective_token)
...
async def serve_chat_interface():
    ...
    This avoids CORS issues by serving the frontend from the same origin as the API.
    """
...
async def register_user(user: UserRegister, response: Response):
    """
    Register a user with their name and create a new session.
    
    This endpoint:
    1. Creates a new chat session
    2. Generates a JWT token with user name and session ID
    3. Sets the token as an HttpOnly cookie
    
    The cookie is automatically sent by the browser on subsequent requests
    when using credentials: 'include' (fetch) or withCredentials: true (EventSource).
    
    Args:
        user: UserRegister model containing the user's name
        response: FastAPI Response object for setting cookies
    
    Returns:
        Registration success message with session info
    """
    ...
    # Create JWT token containing user info and session
    token = create_token(user.name.strip(), session_id)
    
    # Set HttpOnly cookie - secure and not accessible by JavaScript
    response.set_cookie(
        key="auth_token",
        value=token,
        httponly=True,           # JavaScript cannot access this cookie (XSS protection)
        secure=False,            # Set to True in production (requires HTTPS)
        samesite="lax",          # CSRF protection - cookie sent on same-site requests
        max_age=JWT_EXPIRY_HOURS * 3600,  # Cookie expiry in seconds
        path="/"                 # Important: cookie sent on ALL paths, not just /auth/
    )
    ...
    return {
        ...
        "session_id": session_id,
        "questions_remaining": MAX_QUESTIONS_PER_SESSION,
        "token": token  # Also return token for EventSource fallback (cookie issues in some browsers)
    }
...
async def get_current_user_info(user: dict = Depends(get_current_user)):
    ...
    This endpoint requires authentication via HttpOnly cookie.
    Useful for checking if the user is still logged in and getting their session info.
    
    Args:
        user: Current user info extracted from the JWT token in the cookie
    
    Returns:
        User information including name, session ID, and questions remaining
    """
    session_id = user["session_id"]
    
    # Calculate remaining questions
    if session_id in conversation_sessions:
        user_questions = sum(1 for msg in conversation_sessions[session_id] if msg["role"] == "user")
        questions_remaining = MAX_QUESTIONS_PER_SESSION - user_questions
    else:
        questions_remaining = MAX_QUESTIONS_PER_SESSION
...
async def get_history(session_id: str, user: dict = Depends(get_current_user)):
    ...
    Requires authentication and user must own the session.
    """
    # Verify user owns this session
    if user["session_id"] != session_id:
        raise HTTPException(status_code=403, detail="You can only access your own session history")
...
async def chat_stream_post(
    ...
    return await _chat_stream_handler(
        session_id=user["session_id"],  # Use session from authenticated user
...
async def chat_stream_get(
    ...
    This endpoint is designed for use with EventSource.
    Requires authentication via HttpOnly cookie (withCredentials: true).
    """
    if not message:
...
async def _chat_stream_handler(
    ...
    Args:
        session_id: The authenticated user's session ID (from JWT token)
        message: The user's question/message
        use_query_expansion: Whether to use query optimization
    
    Rate Limiting:
        Each session is limited to MAX_QUESTIONS_PER_SESSION questions.
        Once the limit is reached, returns HTTP 429 (Too Many Requests).
    """
    ...
    # Check session question limit before processing
    # Count only user messages (not assistant responses) to determine questions asked
    if session_id in conversation_sessions:
        user_questions = sum(1 for msg in conversation_sessions[session_id] if msg["role"] == "user")
        if user_questions >= MAX_QUESTIONS_PER_SESSION:
            raise HTTPException(
                status_code=429,
                detail=f"Session limit reached. Maximum {MAX_QUESTIONS_PER_SESSION} questions per session. Please start a new session."
            )
    
    question = message
    use_query_expansion = use_query_expansion
    
    # Calculate remaining questions for this session (used in response)
    current_question_count = sum(1 for msg in conversation_sessions.get(session_id, []) if msg["role"] == "user")
    questions_remaining = MAX_QUESTIONS_PER_SESSION - current_question_count - 1  # -1 for current question
    
    async def event_generator():
        try:
            # Send session info with questions remaining count
            yield f"data: {json.dumps({'type': 'session', 'session_id': session_id, 'questions_remaining': questions_remaining})}\n\n"
